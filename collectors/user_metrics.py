...
class UserMetricsCollector:
    ...
    def __init__(self, db):
        ...
        self.user_last_active = Gauge('openwebui_user_last_active_seconds', 
                                    'Timestamp of last user activity', ['user_id'])
    ...
    def collect_metrics(self):
        ...
        try:
            with self.db.cursor() as cur:
                ...
                cur.execute("SELECT id, last_active_at FROM public.user")
                for user_id, last_active in cur.fetchall():
                    self.user_last_active.labels(user_id=user_id).set(last_active)
